...
type ProcessStatus = "idle" | "uploading" | "analyzing" | "zkVerifying" | "minting" | "complete";
...
export function FactorTab({ factoryContract, nftContract }: FactorTabProps) {
    ...
    // Compliance contract for ZK verification
    ...
    const [gstHash, setGstHash] = useState<string>("");
    ...
    // ZK Proof Verification - Gatekeeper Pattern
    const handleZkVerify = async () => {
        if (!account) return;
        setStatus("zkVerifying");
        setError(null);

        try {
            // Generate deterministic GST hash from supplier wallet address
            // In production, this would be generated from actual GSTIN data
            const generatedGstHash = ethers.keccak256(
                ethers.solidityPacked(
                    ["string", "address"],
                    ["GST_VERIFICATION_", account.address]
                )
            );
            setGstHash(generatedGstHash);

            // Mock Groth16 proof structure (a, b, c points)
            // In production, these would be generated by snarkjs from a Circom circuit
            const mockProofA: [bigint, bigint] = [BigInt(1), BigInt(2)];
            const mockProofB: [[bigint, bigint], [bigint, bigint]] = [
                [BigInt(1), BigInt(2)],
                [BigInt(3), BigInt(4)]
            ];
            const mockProofC: [bigint, bigint] = [BigInt(1), BigInt(2)];

            // Public signals: [0] = GST hash converted to uint256
            const pubSignals: bigint[] = [BigInt(generatedGstHash)];

            const transaction = prepareContractCall({
                contract: complianceContract,
                method: "function verifyGSTProof(uint256[2] memory a, uint256[2][2] memory b, uint256[2] memory c, uint256[] memory pubSignals)",
                params: [mockProofA, mockProofB, mockProofC, pubSignals],
            });
            ...
            sendTx(transaction, {
                onSuccess: () => {
                    ...
                    toast.success("ZK Proof Verified!", {
                        description: "Your GST compliance has been verified on-chain."
                    });
                },
                onError: (err: any) => {
                    console.error("ZK verification failed:", err);
                    const msg = err.message?.includes("execution reverted")
                        ? "ZK verification failed. Proof invalid."
                        : "ZK verification transaction failed.";
                    setError(msg);
                    toast.error(msg);
                    setStatus("idle");
                },
            ...
            setError("Failed to prepare ZK verification transaction");
            setStatus("idle");
        }
    };
    ...
    const handleMint = async () => {
        if (!account || !signature || !isZkVerified || !gstHash) return;
        setStatus("minting");
        setError(null);
        setTxStep("current");
        setGasEstimate(null);
    ...
    const getStepStatus = (step: "upload" | "analyze" | "zkVerify" | "mint") => {
        const map: Record<ProcessStatus, Record<string, "idle" | "active" | "complete">> = {
            idle: { upload: "idle", analyze: "idle", zkVerify: "idle", mint: "idle" },
            uploading: { upload: "active", analyze: "idle", zkVerify: "idle", mint: "idle" },
            analyzing: { upload: "complete", analyze: "active", zkVerify: "idle", mint: "idle" },
            zkVerifying: { upload: "complete", analyze: "complete", zkVerify: "active", mint: "idle" },
            minting: { upload: "complete", analyze: "complete", zkVerify: "complete", mint: "active" },
            complete: { upload: "complete", analyze: "complete", zkVerify: "complete", mint: "complete" },
        };
        // Override zkVerify status if already verified
        if (isZkVerified && step === "zkVerify") return "complete";
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="grid grid-cols-4 gap-4">
                ...
                <ProcessStep step={3} label="ZK Verify" icon={ShieldCheck} status={getStepStatus("zkVerify")} />
                <ProcessStep step={4} label="Mint Token" icon={Coins} status={getStepStatus("mint")} isLast />
            </div>
                            ...
                            {status === "complete" ? (
                                ...
                                >
                                    {status === "uploading" || status === "analyzing" ? (
                                        ...
                                        <>
                                            Analyze Invoice <ArrowRight className="ml-2 h-5 w-5" />
                                        </>
                                    )}
                                </Button>
                            ) : !isZkVerified ? (
                                <div className="space-y-4">
                                    <div className="flex items-center gap-2 text-primary bg-primary/10 border border-primary/20 rounded-lg p-3 text-sm">
                                        <ShieldCheck className="h-4 w-4" />
                                        AI Analysis passed. Ready for ZK privacy verification.
                                    </div>
                                    <Button
                                        onClick={handleZkVerify}
                                        disabled={status === "zkVerifying"}
                                        className="w-full h-12 text-lg shadow-lg shadow-primary/20"
                                        size="lg"
                                    >
                                        {status === "zkVerifying" ? (
